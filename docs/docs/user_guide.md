...
The `a2rchi restart` command allows you to restart individual services/containers in an existing deployment without restarting the entire stack. This is particularly useful during development when you want to test changes to specific services (e.g., new agents or pipelines) without restarting data-intensive services like `data_manager`, which handles scraping and vector store population.

**Basic usage:**
```bash
a2rchi restart -n <deployment-name> --service <service-name>
```

**Example:**
```bash
a2rchi restart -n test --service chatbot -c my_config.yaml --no-build -e secrets.env -p -v 4
```

**Available options:**

- `--name` / `-n`: (Required) Name of the existing deployment
- `--service` / `-s`: Service to restart (default: `chatbot`)
- `--config` / `-c`: Path to updated .yaml configuration file(s) (can be specified multiple times)
- `--config-dir` / `-cd`: Path to directory containing configuration files
- `--env-file` / `-e`: Path to .env file with secrets
...
- `--verbosity` / `-v`: Logging verbosity level (0-4, default: 3)
